...
- **Status:** 03a, 03b, 04 fully restyled. **Pending:** nb05 (OAT tornado, scenario plots, separation-shift bars, obligation-regime charts) and the Streamlit app fan charts (still on the old crimson/dimgray + signed axes). nb04 was edited via scripted JSON cell-replacement (it exceeds the Read-tool size limit) and the edited cells' outputs were cleared — Run All to regenerate.
