...
**Presentation conventions:** Cross-profile displays (charts, pivots, printed tables) use the order Enlisted → Prior-Enlisted Officer → Officer via a `PROFILE_ORDER` constant in each notebook; computation/run loops keep their original order to preserve per-scenario seeds and MC draw sequences. Profile colors (University of Michigan palette): Enlisted Ross orange `#D86018`, PEO Rackham green `#75988d`, Officer Matthaei violet `#575294`. Literal dollars in **notebook markdown** use the HTML entity `&#36;` (e.g. `&#36;702K`): paired `$` otherwise render as LaTeX/MathJax, and while `\$` escapes correctly in VS Code/Jupyter, GitHub's `.ipynb` renderer does **not** honor `\$` and mangles the text — `&#36;` renders as a plain `$` everywhere. In **`README.md`** (rendered by GitHub's Markdown engine, which does honor the escape) keep `\$`. In **notebook code cells**, matplotlib mathtext labels keep `\$` inside raw f-strings (`rf"\${x}K"`) — that is unrelated to the markdown convention.
